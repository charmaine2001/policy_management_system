...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased text-gray-900" style="background:#f8fafc;">
        <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false, sidebarCollapsed: false }">
            ...
            <aside
                ...
                class="fixed inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-in-out bg-white border-r border-gray-100 lg:translate-x-0 lg:static lg:inset-0"
                aria-label="Sidebar navigation"
            >
                <!-- Logo Section -->
                <div class="relative flex items-center h-20 px-8 border-b border-gray-50 flex-shrink-0">
                    <div class="flex items-center gap-4">
                        ...
                        <div x-show="!sidebarCollapsed" x-transition class="overflow-hidden whitespace-nowrap">
                            <span class="text-gray-900 font-black text-xl tracking-tight block">Zimnat</span>
                            <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest block -mt-1">Policy System</span>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="!sidebarCollapsed" x-transition class="px-8 py-8">
                    <div class="p-5 rounded-2xl bg-gray-50 border border-gray-100 relative overflow-hidden group">
                        ...
                        <div class="relative flex flex-col gap-3">
                            <div class="w-12 h-12 rounded-2xl bg-white border border-gray-200 flex items-center justify-center text-zimnat-blue font-black text-lg shadow-sm">
                                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                            </div>
                            <div>
                                <p class="text-gray-900 font-black text-sm uppercase tracking-tight">{{ Auth::user()->name }}</p>
                ...
                <nav class="flex-1 px-4 space-y-2 overflow-y-auto">
                    <a href="{{ route('dashboard') }}"
                       class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all group {{ request()->routeIs('dashboard') ? 'bg-gray-50 text-zimnat-blue border-l-4 border-zimnat-blue rounded-l-none' : 'text-gray-500 hover:bg-gray-50 hover:text-zimnat-blue' }}"
                    >
                    ...
                    @if(Auth::user()->role == 'admin' || Auth::user()->role == 'policy_officer')
                        <a href="{{ route('policies.index') }}"
                           class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all group {{ request()->routeIs('policies.*') ? 'bg-gray-50 text-zimnat-blue border-l-4 border-zimnat-blue rounded-l-none' : 'text-gray-500 hover:bg-gray-50 hover:text-zimnat-blue' }}"
                        >
                            <div class="w-5 h-5 flex items-center justify-center">
                        ...
                        <a href="{{ route('queries.index') }}"
                           class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all group {{ request()->routeIs('queries.*') ? 'bg-gray-50 text-zimnat-blue border-l-4 border-zimnat-blue rounded-l-none' : 'text-gray-500 hover:bg-gray-50 hover:text-zimnat-blue' }}"
                        >
                            <div class="w-5 h-5 flex items-center justify-center">
                    ...
                    @if(Auth::user()->role == 'admin')
                        <a href="{{ route('users.index') }}"
                           class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all group {{ request()->routeIs('users.*') ? 'bg-gray-50 text-zimnat-blue border-l-4 border-zimnat-blue rounded-l-none' : 'text-gray-500 hover:bg-gray-50 hover:text-zimnat-blue' }}"
                        >
                            <div class="w-5 h-5 flex items-center justify-center">
                    ...
                    <a href="{{ route('sop') }}"
                       class="flex items-center gap-4 px-4 py-4 rounded-2xl transition-all group {{ request()->routeIs('sop') ? 'bg-gray-50 text-zimnat-blue border-l-4 border-zimnat-blue rounded-l-none' : 'text-gray-500 hover:bg-gray-50 hover:text-zimnat-blue' }}"
                    >
                        <div class="w-5 h-5 flex items-center justify-center">
                        ...
                        <span x-show="!sidebarCollapsed" class="font-black text-[11px] uppercase tracking-widest">User SOP</span>
                    </a>
                </nav>
            ...
            <div class="flex-1 flex flex-col overflow-hidden min-w-0">
                ...
                <!-- Top Navbar -->
                <header class="h-16 flex items-center justify-between px-6 bg-white border-b border-gray-200 z-10">
                    <div class="flex items-center gap-4">
                        <button @click="sidebarOpen = true" class="p-2 text-gray-500 hover:bg-gray-100 rounded-lg lg:hidden">
                            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6H20M4 12H20M4 18H20"/></svg>
                        </button>
                        <button @click="sidebarCollapsed = !sidebarCollapsed" class="hidden lg:flex p-2 text-gray-400 hover:text-zimnat-blue hover:bg-blue-50 rounded-lg transition-colors">
                    ...
                    <div class="flex items-center">
                        @isset($header)
                            <h2 class="text-[11px] font-black text-gray-400 uppercase tracking-[0.3em]">{{ $header }}</h2>
                        @endisset
                    </div>
                    ...
                    <div class="flex items-center gap-5">
                        ...
                        <div x-data="{ userMenu: false }" class="relative">
                            <button @click="userMenu = !userMenu" class="flex items-center gap-3 p-1.5 pr-4 rounded-xl hover:bg-gray-50 transition-all border border-transparent hover:border-gray-100 shadow-sm hover:shadow-md group">
                                <div class="w-8 h-8 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center text-zimnat-green font-black text-xs shadow-inner group-hover:bg-white transition-colors">
                                    {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                                </div>
                                <div class="hidden md:block text-left">
                                    <p class="text-[11px] font-black text-gray-900 uppercase tracking-tighter">{{ Auth::user()->name }}</p>
                                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest -mt-0.5">{{ str_replace('_', ' ', Auth::user()->role) }}</p>
                                </div>
                            ...
                            <div x-show="userMenu" 
                                 ...
                                 class="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 py-3 z-50 overflow-hidden" 
                            >
                                ...
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-5 py-2.5 text-xs font-black text-gray-600 uppercase tracking-widest hover:bg-gray-50 hover:text-zimnat-blue transition-colors">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto" style="background: #f8fafc;">
